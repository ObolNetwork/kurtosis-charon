...
# Function to extract Kubernetes service endpoints with specific port
extract_and_save_beacon_endpoints() {
    local namespace=$1
    local port_filter=4000
    local endpoints=()

    echo "Extracting beacon node endpoints in namespace: $namespace with port $port_filter"
    ...
    while IFS= read -r service; do
        ...
        ports=$(echo "$service" | jq -c '.spec.ports[]?')

        # Loop through all ports for the service
        while IFS= read -r port; do
            port_number=$(echo "$port" | jq -r '.port')

            # Only include services with the specific port
            if [[ "$port_number" -eq "$port_filter" ]]; then
                fqdn="http://${name}.${namespace}.svc.cluster.local:${port_number}"
                echo "Adding endpoint: $fqdn"
                endpoints+=("$fqdn")
            fi
        done <<< "$ports"
    done <<< "$(echo "$services" | jq -c '.items[]')"

    # Create comma-separated string from endpoints array
    local endpoints_string=$(IFS=,; echo "${endpoints[*]}")
    
    # Write to .env file
    if [ -f ".env" ]; then
        # Remove existing CHARON_BEACON_NODE_ENDPOINTS line if it exists
        sed -i '' '/^CHARON_BEACON_NODE_ENDPOINTS=/d' .env
    fi
    echo "CHARON_BEACON_NODE_ENDPOINTS=$endpoints_string" >> .env
    echo "Saved beacon endpoints to .env file"
}
...
# Function to update or add a value in YAML file
update_yaml_value() {
    local file=$1
    local key=$2
    local value=$3
    ...
    if [ ! -f "$file" ]; then
        echo "Creating new YAML file: $file"
        touch "$file"
    fi
    
    # Check if key exists
    if grep -q "^${key}:" "$file"; then
        # On macOS, we need to use sed with an empty string for -i
...
#print the beaconClients
echo "Beacon Clients: ${beaconClients[@]}"
...
echo "Validator Keystore Files Artifact UUID: $uuidValidator"
    ...
    if [ -n "${cl_port}" ]; then
        ...
        # Update my-values.yaml with the genesis time
        update_yaml_value "my-values.yaml" "TESTNET_GENESIS_TIME_STAMP" "\"$genesis_time\""
    else
        echo "Port not found."
    fi
else
...
ENV_FILE="./.env"
extract_and_save_beacon_endpoints "$NAMESPACE" bnendpoints
...
rm -rf charon-keys
rm .env
kubectl delete service prometheus -n kt-${CLUSTER_NAME}
